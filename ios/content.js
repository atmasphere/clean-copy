// Clean Copy — content script (Safari/iOS compatible)
...
// navigator.clipboard.writeText fails in content scripts on iOS Safari.
// Fall back to execCommand('copy') with a temporary textarea.

function copyToClipboard(text) {
  // Try the modern API first (works on desktop Safari and Chrome)
...
function execCommandCopy(text) {
  return new Promise((resolve, reject) => {
    ...
    textarea.value = text;
    // Must be visible and in viewport for iOS
...
// Detect iOS/mobile for touch-friendly behavior
...
function getOrCreatePanel() {
  ...
  shadow.innerHTML = `
    <style>
      :host {
        ...
        top: 0;
        right: 0;
        width: ${IS_MOBILE ? "100vw" : PANEL_DEFAULT_WIDTH + "px"};
        height: 100vh;
        height: 100dvh;
        z-index: 2147483647;
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .toolbar {
        display: flex;
        align-items: center;
        gap: ${IS_MOBILE ? "8px" : "6px"};
        padding: ${IS_MOBILE ? "10px 14px" : "8px 12px"};
        border-bottom: 1px solid var(--border);
        font-family: system-ui, -apple-system, sans-serif;
        font-size: ${IS_MOBILE ? "14px" : "12px"};
        flex-shrink: 0;
        background: var(--bg);
        padding-top: max(${IS_MOBILE ? "10px" : "8px"}, env(safe-area-inset-top));
        flex-wrap: ${IS_MOBILE ? "wrap" : "nowrap"};
      }
      .toolbar button, .toolbar select {
        background: none;
        border: 1px solid var(--border);
        border-radius: ${IS_MOBILE ? "8px" : "4px"};
        color: var(--text);
        padding: ${IS_MOBILE ? "8px 12px" : "3px 8px"};
        font-size: ${IS_MOBILE ? "14px" : "11px"};
        cursor: pointer;
        font-family: system-ui, sans-serif;
        line-height: 1.4;
        min-height: ${IS_MOBILE ? "40px" : "auto"};
      ...
      .theme-btn {
        min-width: ${IS_MOBILE ? "40px" : "28px"};
        text-align: center;
        font-size: ${IS_MOBILE ? "18px" : "14px"} !important;
      }
      .close-btn {
        border: none !important;
        font-size: ${IS_MOBILE ? "22px" : "16px"} !important;
        padding: ${IS_MOBILE ? "4px 10px" : "2px 6px"} !important;
        line-height: 1 !important;
        min-width: ${IS_MOBILE ? "40px" : "auto"};
      }
      ...
      .archive-btn {
        ...
        padding: ${IS_MOBILE ? "12px 20px" : "8px 16px"};
        background: var(--link);
        color: var(--bg);
        border: none;
        border-radius: ${IS_MOBILE ? "10px" : "6px"};
        font-size: ${IS_MOBILE ? "15px" : "13px"};
        font-family: system-ui, sans-serif;
        cursor: pointer;
        -webkit-tap-highlight-color: transparent;
        touch-action: manipulation;
      }
      .archive-btn:active { opacity: 0.7; }
      ...
      /* Resize handle — hidden on mobile (panel is full width) */
      .resize-handle {
        position: absolute;
        top: 0;
        left: 0;
  ...
  // Copy buttons — use iOS-safe clipboard helper
  root.querySelector(".copy-md").addEventListener("click", () => {
    const article = panel._articleData;
    if (!article) return;
    copyToClipboard(articleToMarkdown(article)).then(() => showToast("Copied as Markdown"));
  });

  root.querySelector(".copy-text").addEventListener("click", () => {
    const article = panel._articleData;
    if (!article) return;
    copyToClipboard(articleToPlainText(article)).then(() => showToast("Copied as plain text"));
  });
...
// Archive fetch — resilient to background script death on iOS.
// If sendMessage fails (background is dead), fall back to manual links.
async function loadFromArchive(panel, contentEl, metaParts) {
  ...
  try {
    ...
    // Background script is dead (common on iOS Safari).
    // Fall back to manual archive links.
    result = { ok: false, error: "background_dead" };
  }
  ...
  if (!result || !result.ok) {
    ...
    if (prompt) {
      prompt.innerHTML = `
        <p>Couldn't fetch automatically — ${result && result.error === "background_dead" ? "the background process isn't running" : "archive.today may be blocking the request"}.</p>
...
function showArchiveBanner() {
  ...
  shadow.innerHTML = `
    <style>
      ...
      .banner button {
        padding: ${IS_MOBILE ? "10px 18px" : "5px 14px"};
        border: none;
        border-radius: ${IS_MOBILE ? "8px" : "5px"};
        font-size: ${IS_MOBILE ? "15px" : "12px"};
        font-family: system-ui, sans-serif;
        cursor: pointer;
        -webkit-tap-highlight-color: transparent;
        touch-action: manipulation;
